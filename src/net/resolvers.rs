...
// Explicit cleanup only; there is no timer or background score table.
pub fn invalidate_network_caches() {
    if let Ok(mut cache) = DNS_PACKET_CACHE.lock() {
        *cache = None;
    }
    if let Ok(mut cache) = CHOICE.lock() {
        *cache = None;
    }
    if let Ok(mut cache) = LIVENESS.lock() {
        *cache = None;
    }
    if let Ok(mut cache) = PROXY_SET.lock() {
        *cache = None;
    }
}
