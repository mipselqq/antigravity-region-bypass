...
После обновления с 2.3.0/2.3.1 выберите **«Включить обход»** и полностью перезапустите Antigravity и терминалы CLI. Программа восстанавливает сохранённые настройки локального шлюза и возвращает прямой endpoint. Собственные адреса пользователя сохраняются. Команда `speed` показывает используемый режим без сетевых замеров.
